...
- Euler `work` to SAN for WebDataset shards `sbatch -n 4 --cpus-per-task=4 --time=4:00:00 --mem-per-cpu=1024 --wrap="rsync -ahv --include='*/' --include='*.tar' --exclude='*' /cluster/work/refregier/athomsen/CosmoGrid/DESY3/v4/large_scales/webdatasets athomsen@login.phys.ethz.ch:/home/ipa/refreg/experiments/athomsen/CosmoGrid/DESY3/v4/large_scales"` (or Globus). Use the legacy TFRecord transfer only for old runs that still consume `.tfrecord` files.
